...
public class PatientFhirResourceDao extends BaseFhirResourceDao<Patient> {
            ...
            public Predicate translatePredicateString(Class<? extends IResourceEntity> entity, String theParamName, String likeExpression,
                ...
                switch (theParamName) {
                    ...
                    case Patient.SP_NAME:
                        singleCode = decomposeName(likeExpression, from, theBuilder);
                        break;
                    default:
                        break;
                }
    ...
    private Predicate decomposeName(final String name,
                                    From<? extends IResourceEntity, ? extends IResourceEntity> from,
                                    final CriteriaBuilder builder) {
        final String parts[] = name.substring(1, name.length() - 1).split(" ");
        Predicate predicate = null;

        for (final String part : parts) {
            final String value = format("%%%s%%", part);
            final Predicate gn1 = builder.like(from.get("givenName1").as(String.class), value);
            final Predicate gn2 = builder.like(from.get("givenName2").as(String.class), value);
            Predicate fn1 = builder.like(from.get("familyName").as(String.class), value);
            Predicate n1 = builder.like(from.get("prefixName").as(String.class), value);
            Predicate n2 = builder.like(from.get("suffixName").as(String.class), value);
            if (predicate == null) predicate = builder.or(gn1, gn2, fn1, n1, n2);
            else predicate = builder.and(predicate, builder.or(gn1, gn2, fn1, n1, n2));
        }
